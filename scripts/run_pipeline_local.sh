...
# ====== 3) Moonshots (Kraken-only in één stap) ======
...
# ====== 4) Market regime + cooldown + allocatie ======
...
# ====== 5) Archiveren naar Desktop + history (met datum) ======
bash scripts/archive_reports.sh "$REPORTS_DIR" "$DESKTOP_DIR"

# Extra: zet ook de Kraken-bestanden direct op je Desktop en in history met datum
cp -f "$REPORTS_DIR"/scores_kraken_latest.*     "$DESKTOP_DIR"/
cp -f "$REPORTS_DIR"/moonshots_kraken_latest.*  "$DESKTOP_DIR"/
...
echo "✅ Klaar. Bekijk: $DESKTOP_DIR  en  $HISTORY_DIR (datumstempels: ${STAMP}_*)"
